...
import {
  MangaFavouritesFragment,
  useGetMangaFavouritesQuery,
...
export default function MangaFavourites({
  data,
  userId,
  perPage,
}: {
  data: MangaFavouritesFragment;
  userId: number;
  perPage: number;
}) {
  ...
  const { data: fetchedData, loading } = useGetMangaFavouritesQuery({
    variables: { userId, page: currentPage, favouritesPerPage: perPage },
    skip: currentPage === 1,
  });

  useEffect(() => {
    if (fetchedData?.User?.favourites?.manga?.nodes) {
      setDisplayData((prev) => ({
        pageInfo: {
          hasNextPage:
  ...
  useEffect(() => {
    if (toggleFavouritesData) {
      setDisplayData((prev) => {
        return {
          ...prev,
          nodes: toggleFavouritesData.ToggleFavourite?.manga?.nodes,
        };
      });
    }
        ...
        displayData.nodes?.map((media) => (
          ...
          >
            ...
            <button
              ...
              onClick={(e) => {
                e.preventDefault();
                toggleFavourites({ variables: { mangaId: media?.id } });
              }}
            >
              &#x2716;
            </button>
